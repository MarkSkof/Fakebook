'use client';

import { useState } from "react";
import SearchBar from "../SearchBar";
import { UploadedFilePost } from "../UploadedPost";

export default function SavedPostsPage() {
  const userCount = 3;
  const ids = [...Array(userCount).keys()];
  const names = [
    "Bob", "Eve", "Ivy", 
  ];
  const imgPaths = [    
    "https://yt3.googleusercontent.com/yuTu8Ba-3xldAMURGkQXLkZtIH6764LXBHAKisYyN3BdtQG0BMqqrUDwVnEskLJhUyDgyFrH=s900-c-k-c0x00ffffff-no-rj",
    "https://d2v5dzhdg4zhx3.cloudfront.net/web-assets/images/storypages/short/linkedin-profile-picture-maker/dummy_image/thumb/004.webp",
    "data:image/jpeg;base64,",
  ];
  const imgTypes = Array(userCount).fill("image");
  const titles = [
    "title2" , "title5" ,"title9"
];
  const likes = [5,  1, 0];
  const postDates = Array(userCount).fill(new Date());

  // Create plain data array
  const uploadedFilePostData = [...Array(userCount)].map((_, index) => ({
    id: ids[index % names.length],
    name: names[index % names.length],
    imgPath: imgPaths[index % imgPaths.length],
    title: titles[index % titles.length],
    fileUrl: imgPaths[index % imgPaths.length],
    fileType: imgTypes[index % imgTypes.length],
    postDate: postDates[index % postDates.length],
    likeCount: likes[index % likes.length],
    dislikeCount: likes[index % likes.length],
    comments: []
  }));

  const [searchQuery, setSearchQuery] = useState("");

  // Filter the data
  const filteredUploadedFilePostData = uploadedFilePostData.filter(post =>
    post.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    post.title.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // Map filtered data to JSX
  const filteredUploadedFilePostList = filteredUploadedFilePostData.map((post, index) => (
    <UploadedFilePost
      key={index}
      id={post.id}
      name={post.name}
      imgPath={post.imgPath}
      title={post.title}
      fileUrl={post.fileUrl}
      fileType={post.fileType}
      postDate={post.postDate}
      likeCount={post.likeCount}
      dislikeCount={post.dislikeCount}
      comments={post.comments}
    />
  ));

  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSearchQuery(e.target.value);
  };

  return (
    <div className="p-4 w-4/6 h-screen">
      <div>
        <h3 className="font-bold mb-4">Saved Posts</h3>
      </div>
      <div>
        <SearchBar
          placeHolder="Search..."
          searchQuery={searchQuery}
          onSearchChange={handleSearchChange}
        />
      </div>
      <div className="bg-white p-4 rounded-lg shadow mb-4">
        <div className="posts">
          {filteredUploadedFilePostList}
        </div>
      </div>
      <style>{`
      .posts {
        display: flex;
        flex-wrap: wrap;
        gap: 10px;
      }`}</style>
    </div>
  );
}
